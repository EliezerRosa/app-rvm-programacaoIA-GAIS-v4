...
export async function parseScheduleFromPdf(base64Pdf: string): Promise<ParsedPart[]> {
    ...
    if (prayerIndices.length > 0) {
        ...
        // Adiciona a Oração Final se ela não for a mesma que a Oração Inicial,
        // ou se não houver uma Oração Inicial identificada.
    ...
    // Adiciona o Leitor do EBC se houver um Dirigente na pauta.
